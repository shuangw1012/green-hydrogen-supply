...
def obtain_CoV(Location):
    weather_data_folder = datadir + os.sep + 'SAM_INPUTS' + os.sep + 'WEATHER_DATA'
    file1 = weather_data_folder + os.sep + 'weather_data_%s.csv'%Location
    wspd1 = pd.read_csv(file1)['Pressure Units'].values[2:].astype(float)
    mean = np.mean(wspd1)
    std_dev = np.std(wspd1)
...
if __name__=='__main__':
    ...
    wind_cf = wind_output(Location)
    solar_cf = solar_output(Location)
    cv = obtain_CoV(Location)
