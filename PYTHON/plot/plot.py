...
def plot_bar2():
    ...
    Filter_location = ['Sep-optimal','Location-specific']
    cost_categories = ['PV','Wind', 'Electrolyser', 'Storage', 'Transmission', 'Transportation']
  
    data_multi = np.array([[0.62,1.59,1.26,0.09,0.01,0.27],
                           [0.58,2.33,1.34,0.09,0.01,0.27]])
...
def plot_GIS():
    ...
    Results = pd.read_csv(os.path.join(os.getcwd(), 'results_2020_salt1.csv'))
    data = pd.read_csv(os.path.join(os.getcwd(), 'input_Pilbara_plot.txt'))
    notation = True
    for k in range(len(Results['El'].values)):
        ...
        # Create a Folium map
        m = folium.Map(location=[-21.85,119.13], zoom_start=9)
...
def plot_GIS_stg():
    ...
    from folium.features import DivIcon
    # Load your data from the CSV file
    Results = pd.read_csv(os.path.join(os.getcwd(), 'results_2020_sep2.csv'))
    ...
    data = pd.read_csv(os.path.join(os.getcwd(), 'input_Pilbara_sep.txt'))#'input_usg_plot_salt1.txt'))
    notation = True
    for k in range(len(Results['El'].values)):
        #if data['#Name'][k]!= 'KF249':
        #    continue
        ...
        # Create a Folium map
        centre_lat = -24.03#,-22.3#-20.5#-28.11#,
        centre_long = 117.09#119.5#119.1# 119.3#140.23#
        zoom = 10#10#9
        ...
        for i, row in geodata.iterrows():
            if row['#Name'] != 'Electrolyser' and row['#Name'] != 'End user' and row['#Name'] != 'Storage':
                ...
                if float(geodata['K_PV'][i]) > 0.1 and float(geodata['K_wind'][i]) > 0.1:
                    pv_icon = folium.features.CustomIcon('%s/Icon/solar-power.png'%os.getcwd(), icon_size=(30,30))
                    folium.Marker(
                        location=[row['geometry'].y+0.015+delta_y, row['geometry'].x-0.015+delta_x],
                        icon=pv_icon,
                        popup=row['#Name'],
                    ).add_to(m)
                    turbine_icon = folium.features.CustomIcon('%s/Icon/wind-power.png'%os.getcwd(), icon_size=(30,30))
                    folium.Marker(
                        location=[row['geometry'].y-0.015+delta_y, row['geometry'].x+0.015+delta_x],
                        icon=turbine_icon,
                        popup=row['#Name'],
                    ...
                    pv_icon = folium.features.CustomIcon('%s/Icon/solar-power.png'%os.getcwd(), icon_size=(50,50))
                    folium.Marker(
                        location=[row['geometry'].y+delta_y, row['geometry'].x+delta_x],
                        icon=pv_icon,
                        popup=row['#Name'],
                ...
                if notation == True:
                    ...
                    folium.Marker(
                        location=[row['geometry'].y-0.03, row['geometry'].x-0.15],
                        icon=DivIcon(
                            html='<<div style="font-size: 14pt"><b>%s</b></div>' % character,
                            ),
                        popup=row['#Name'],  # Use the "Name" column as the label
